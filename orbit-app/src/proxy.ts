...
function lastMatchNumber(query: string, regex: RegExp) {
  ...
  return matches.length
    ? numeric(matches[matches.length - 1]?.[1])
    : undefined;
}
...
async function parseCriteria(
  ...
  const q = norm(query);
  const location = await detectLocationWithAI(
    query,
    process.env.OPENAI_API_KEY,
  );

  const explicitMinSurface = lastMatchNumber(
    q,
    /(?:surface\s*)?(?:minimum|min\.?|au moins|at least)\s*(\d{2,4}(?:[.,]\d+)?)\s*(?:m2|m²|sqm|sq\s*m)/gi,
  );
  const explicitMaxSurface = lastMatchNumber(
    q,
    /(?:surface\s*)?(?:maximum|max\.?|jusqu(?:a|')a|up to)\s*(\d{2,4}(?:[.,]\d+)?)\s*(?:m2|m²|sqm|sq\s*m)/gi,
  );
  const anySurface = lastMatchNumber(
    q,
    /(\d{2,4}(?:[.,]\d+)?)\s*(?:m2|m²|metres? carres?|sqm|sq\s*m)/gi,
  );
  const bedrooms = lastMatchNumber(
    q,
    /(\d{1,2})\s*(?:chambres?|bedrooms?|beds?)/gi,
  );
  const bathrooms = lastMatchNumber(
    q,
    /(\d{1,2})\s*(?:sdb|salles? de bain|bathrooms?|baths?)/gi,
  );
  const explicitBudgetMin = lastMatchNumber(
    q,
    /(?:prix\s*)?(?:minimum|min\.?|au moins|at least)\s*([\d\s.,]+)\s*(?:€|eur|euros?|\$|usd|£|gbp)?/gi,
  );
  const explicitBudgetMax = lastMatchNumber(
    q,
    /(?:moins de|sous|budget(?: max)?|prix\s*max(?:imum)?|maximum|max\.?|under|below|up to)\s*([\d\s.,]+)\s*(?:€|eur|euros?|\$|usd|£|gbp)?/gi,
  );
  const bareCurrencyBudget = lastMatchNumber(
    q,
    /([\d\s.,]{4,})\s*(?:€|eur|euros?|\$|usd|£|gbp)/gi,
  );

  const house = /\b(maison|maisons|house|houses|villa|villas|home|homes|haus|casa|huis)\b/i.test(
    q,
  );

  const sortFromQuery: Criteria["sortPriority"] =
    /tri\s+prix\s+croissant|lowest price|prix le plus bas/i.test(q)
      ? "lowest_price"
  ...
  return {
    ...
    budgetMax:
      numeric(overrides.budgetMax) ??
      explicitBudgetMax ??
      (explicitBudgetMin ? undefined : bareCurrencyBudget),
    minSurface:
      numeric(overrides.minSurface) ??
      explicitMinSurface ??
      (explicitMaxSurface ? undefined : anySurface),
    ...
    parking:
      overrides.parking ?? /\b(parking|stationnement|carport)\b/i.test(q),
    requirements: [],
    preferences: [],
...
function parseSurface(text: string, criteria: Criteria) {
  ...
  for (const match of text.matchAll(
    /(\d{2,4}(?:[.,]\d+)?)\s*(?:m²|m2|sqm|sq\s*m)\b/gi,
  )) {
    ...
    if (/\b(terrain|parcelle|land|plot|lot|acre|hectare|garden size|jardin de)\b/.test(context)) {
      continue;
    }
    let score = /\b(surface habitable|living area|living space|interior|floor area|habitable)\b/.test(
      context,
    )
      ? 10
      : 3;
    if (criteria.minSurface) {
      score += Math.max(
        0,
        3 -
          Math.abs(value - criteria.minSurface) /
            Math.max(50, criteria.minSurface),
      );
    }
  ...
  for (const match of text.matchAll(
    /([\d,]{3,7})\s*(?:sq\.?\s*ft|square\s*feet|sqft)\b/gi,
  )) {
    ...
    let score = /\b(living|interior|heated|home size|house size)\b/.test(
      context,
    )
      ? 10
      : 3;
...
function parseBedrooms(text: string) {
  const match = text.match(
    /(\d{1,2})\s*(?:chambres?|bedrooms?|beds?|bed)\b/i,
  );
...
function parseBathrooms(text: string) {
  const match = text.match(
    /(\d{1,2})\s*(?:sdb|salles? de bain|bathrooms?|baths?|bath)\b/i,
  );
  const value = match ? Number(match[1]) : undefined;
...
function detectKind(text: string): Listing["propertyKind"] {
  ...
  if (/\b(appartement|apartment|flat|studio|condo|condominium)\b/.test(q)) {
    return "apartment";
  }
  if (/\b(programme neuf|new build|new development)\b/.test(q)) {
    return "new_build_project";
  }
  if (/\b(maison|house|home|pavillon|detached|townhouse|haus|casa|huis)\b/.test(q)) {
...
function looksLikeCollectionPage(result: SearchResult) {
  ...
  if (/\b\d{2,6}\s+(?:listings|annonces|homes|maisons|properties|biens)\b/.test(combined)) {
    return true;
  }
  if (/\b(?:homes|houses|properties|maisons|biens)\s+(?:for sale|à vendre|a vendre)\b/.test(title) && !/\b(ref|mls|reference|réf|\d{5})\b/.test(title)) {
    return true;
  }
  if (/\b(?:search results|résultats de recherche|annonces immobilières|annonces immobilieres)\b/.test(title)) {
    return true;
  }
  ...
  try {
    ...
    if (segments.length <= 1 && /\b(vente|sale|buy|immobilier|property|homes|maison)\b/.test(title)) {
      return true;
    }
  } catch {}
...
function clearlyIrrelevant(text: string) {
  return /\b(vacation rental|location vacances|hotel|booking|airbnb|emploi|job|actualite|news|wikipedia|toy|doll|poupee|barbie)\b/i.test(
    norm(text),
  );
}
...
function sourceTrust(domain: string, country?: string) {
  ...
  if (preferred.some((item) => domain === item || domain.endsWith(`.${item}`))) {
    return 12;
  }
  if (/properstar|jamesedition|sothebysrealty|engelvoelkers/.test(domain)) {
    return 4;
  }
  return 0;
}
...
function toListing(result: SearchResult, criteria: Criteria): Listing | null {
  ...
  if (criteria.city) {
    matchScore += norm(`${combined} ${result.url}`).includes(norm(criteria.city))
      ? 23
      : -8;
  }
  if (criteria.propertyType === "house") {
    matchScore +=
      kind === "existing_house" || kind === "villa"
        ? 18
        : kind === "apartment"
          ? -30
          : 0;
  ...
  return {
    ...
    title,
    description:
      description || "Informations disponibles depuis le moteur de recherche.",
    ...
    reasons: [],
    compromises: [
      "Prix et photo en cours de vérification sur la page source",
    ],
...
function canonicalFromHtml(html: string) {
  ...
  return canonical ? decodeHtml(canonical) : undefined;
}
...
function validSourceImage(url: unknown, pageHost: string) {
  ...
  if (
    /logo|favicon|icon|avatar|sprite|placeholder|no[-_]?image|default|brand|cookie|tracking|pixel|banner|advert|ads?\b|social-share|og-default|profile|agency|agent|avatar|map|floorplan|plan[-_]?image/.test(
      q,
    )
  ) {
    return false;
  ...
  try {
    ...
    return (
      imageHost === pageHost ||
      imageHost.endsWith(`.${pageHost}`) ||
      /cloudfront|akamai|cdn|imgix|images|media|static|cloudinary|fastly/.test(
        imageHost,
      )
    );
  } catch {
...
function structuredPrice(
  ...
  const currency =
    typeof rawCurrency === "string" && rawCurrency.length <= 5
  ...
  if (criteria.budgetMax && price > criteria.budgetMax * 4) return undefined;
...
function parseSourceDetails(
  ...
  for (const match of html.matchAll(
    ...
    try {
      jsonLdRoots.push(JSON.parse(decodeHtml(match[1] ?? "")));
    } catch {}
  }

  const candidates: Array<Record<string, unknown>> = [];

  const collectEntities = (value: unknown, depth = 0) => {
    if (depth > 5) return;
    if (Array.isArray(value)) {
      for (const item of value) collectEntities(item, depth + 1);
      return;
    }
    ...
    const type = norm(
      Array.isArray(obj["@type"])
        ? (obj["@type"] as unknown[]).join(" ")
        : obj["@type"],
    );

    if (
      /house|residence|singlefamily|apartment|accommodation|realestate|product/.test(
        type,
      )
    ) {
      candidates.push(obj);
    ...
    // @graph/mainEntity are safe page-level relationships.
    // Deliberately do NOT traverse itemListElement: those are usually other ads.
    if (obj["@graph"]) collectEntities(obj["@graph"], depth + 1);
    if (obj.mainEntity) collectEntities(obj.mainEntity, depth + 1);
  };

  for (const root of jsonLdRoots) collectEntities(root);

  const matching = candidates.filter((obj) => {
    const url = entityUrl(obj);
    return url ? sameListingUrl(url, pageUrl) : true;
  });

  const ordered = [...matching].sort((a, b) => {
    const au = entityUrl(a);
    const bu = entityUrl(b);
    const as = au && sameListingUrl(au, pageUrl) ? 1 : 0;
    const bs = bu && sameListingUrl(bu, pageUrl) ? 1 : 0;
    return bs - as;
  });
  ...
  for (const obj of ordered) {
    ...
    const specification = offer?.priceSpecification as
      | Record<string, unknown>
      | undefined;
    ...
    const floor = (obj.floorSize ?? obj.livingArea) as
      | Record<string, unknown>
      | number
      | string
      | undefined;
    ...
    if (floor && typeof floor === "object") {
      ...
      unit = norm(
        (floor as Record<string, unknown>).unitCode ??
          (floor as Record<string, unknown>).unitText,
      );
    }
    ...
    if (!details.surface && fv) {
      ...
      if (sqm >= 15 && sqm <= 1000) {
        details.surface = Math.round(sqm * 10) / 10;
      }
    }
    ...
    if (!details.bedrooms && beds && beds <= 20) {
      details.bedrooms = Math.round(beds);
    }
    if (!details.bathrooms && baths && baths <= 15) {
      details.bathrooms = Math.round(baths);
    }
    ...
    // Once an exact-url entity gave us price + image, stop before related data.
    if (
      entityUrl(obj) &&
      sameListingUrl(entityUrl(obj), pageUrl) &&
      details.price &&
  ...
  if (validSourceImage(ogImage, pageHost)) {
    details.images.push(ogImage!);
  }
...
function sourceText(html: string) {
  ...
  return decodeHtml(`${title} ${metas} ${jsonLd}`)
    .replace(/\s+/g, " ")
    .slice(0, 320_000);
}
...
async function confirmPriceFromIndex(
  ...
  for (const result of results.slice(0, 10)) {
    ...
    const overlap = tokens.filter((token) => norm(text).includes(token)).length;
    const exactUrl = normalizedUrl(result.url) === wantedUrl;

    if (!exactUrl && tokens.length >= 3 && overlap < Math.min(4, tokens.length)) {
      continue;
    }

    const parsed = parsePriceFromText(
      text,
      listing.currency ?? criteria.currency,
    );
    const value = sanitizePropertyPrice(
      parsed.value,
      parsed.currency ?? criteria.currency,
      listing.surface,
      parsed.confidence,
    );
  ...
  const exact = parsedResults.find((item) => item.exactUrl);
  if (exact) return exact;

  const groups = new Map<number, typeof parsedResults>();
  for (const item of parsedResults) {
    const tolerance = Math.max(1000, item.value * 0.005);
    const bucket = Math.round(item.value / tolerance);
...
async function enrichListing(
  ...
  const details = parseSourceDetails(html, criteria, listing.url);
  const text = sourceText(html);
  const surface =
    details.surface ?? parseSurface(text, criteria) ?? listing.surface;
  ...
  for (const image of details.images.slice(0, 3)) {
  ...
  return {
    ...
    surface,
    bedrooms:
      details.bedrooms ?? parseBedrooms(text) ?? listing.bedrooms,
    bathrooms:
      details.bathrooms ?? parseBathrooms(text) ?? listing.bathrooms,
    images: checkedImages,
    pricePerM2:
      price && surface ? Math.round(price / surface) : undefined,
    priceConfidence: confidence,
    reasons: [
      ...(confidence === "confirmed"
        ? ["Prix vérifié sur la page source"]
        : confidence === "indexed"
          ? ["Prix recoupé dans l'index du portail"]
          : []),
      ...(checkedImages.length
        ? ["Photo vérifiée depuis la page de l'annonce"]
        : []),
    ],
    compromises: [
...
function isCleanListing(listing: Listing, criteria: Criteria) {
  if (!listing.price || !listing.images.length) return false;
  if (listing.priceConfidence === "none") return false;
  if (criteria.propertyType === "house" && listing.propertyKind === "apartment") {
    return false;
  }
  if (
    criteria.city &&
    !norm(`${listing.title} ${listing.description} ${listing.url}`).includes(
      norm(criteria.city),
    )
  ) {
    return false;
  ...
  if (
    ...
    listing.surface < criteria.minSurface * 0.85
  ) {
    return false;
  }
  if (
    criteria.maxSurface &&
    listing.surface !== undefined &&
    listing.surface > criteria.maxSurface * 1.12
  ) {
    return false;
  }
  if (
...
async function enrichUntilTen(
  listings: Listing[],
  criteria: Criteria,
) {
  const verified: Listing[] = [];
  const enrichedAll: Listing[] = [];
  const selected = listings.slice(0, MAX_CANDIDATES_TO_ENRICH);

  for (let i = 0; i < selected.length; i += ENRICH_BATCH_SIZE) {
    const batch = await Promise.all(
      selected
        .slice(i, i + ENRICH_BATCH_SIZE)
        .map((listing) => enrichListing(listing, criteria)),
    );

    enrichedAll.push(...batch);
    for (const listing of batch) {
      if (isCleanListing(listing, criteria)) verified.push(listing);
    }

    if (verified.length >= TARGET) break;
  }

  return { verified, enrichedAll };
}

function buildQueries(original: string, criteria: Criteria) {
  const location = criteria.location ?? criteria.city ?? "";
  const terms =
    COUNTRY_TERMS[criteria.country ?? ""] ??
    "house property for sale real estate";
  const details = [
  ...
  const domains = criteria.country
    ? COUNTRY_DOMAINS[criteria.country] ?? []
    : [];

  const portalQueries = domains
    .slice(0, 10)
    .map(
      (domain) =>
        `site:${domain} ${location} ${terms} ${details}`,
    );

  return [
    ...new Set([
      original,
      `${location} ${terms} ${details}`,
      `${location} ${criteria.propertyType ?? "house"} ${details} for sale`,
      ...portalQueries,
    ]),
  ]
    .map(clean)
...
function quality(listing: Listing) {
  ...
  if (listing.priceConfidence === "confirmed") score += 35;
  else if (listing.priceConfidence === "indexed") score += 22;
  if (listing.images.length > 0) score += 25;
  if (listing.surface !== undefined) score += 5;
  if (listing.bedrooms !== undefined) score += 3;
  return score;
}
...
function sortListings(listings: Listing[], criteria: Criteria) {
  ...
  if (criteria.sortPriority === "largest") {
    return [...listings].sort(
      (a, b) =>
        (b.surface ?? 0) - (a.surface ?? 0) ||
        quality(b) - quality(a),
    );
  }

  return [...listings].sort((a, b) => quality(b) - quality(a));
}
  ...
  try {
    ...
    const filters =
      body?.filters && typeof body.filters === "object"
        ? (body.filters as FilterOverrides)
        : {};

    if (!query) {
      return NextResponse.json(
        { success: false, error: "La recherche est vide." },
        { status: 400 },
      );
    }
    ...
    for (let page = 1; page <= 4; page += 1) {
      const searchQueries =
        page === 1 ? queries : queries.slice(0, page === 2 ? 10 : 7);
      const batches = await Promise.all(
        searchQueries.map((q) => searxngSearch(q, page)),
      );
    ...
    candidates.sort((a, b) => b.orbitScore - a.orbitScore);

    const { verified, enrichedAll } = await enrichUntilTen(
      candidates,
      criteria,
    );

    let listings = sortListings(verified, criteria).slice(0, TARGET);

    // In the rare case fewer than 10 fully verified listings are available,
    // keep searching the enriched pool but never re-introduce fake images/prices.
    if (listings.length < TARGET) {
      const safeFallback = enrichedAll
        .filter(
          (listing) =>
            listing.price &&
            listing.images.length > 0 &&
            listing.priceConfidence !== "none" &&
            !listings.some(
              (existing) =>
                normalizedUrl(existing.url) === normalizedUrl(listing.url),
            ),
        )
        .sort((a, b) => quality(b) - quality(a));

      listings = [...listings, ...safeFallback].slice(0, TARGET);
    }
    ...
    return NextResponse.json({
      ...
      analyzedCandidateCount: Math.min(
        candidates.length,
        MAX_CANDIDATES_TO_ENRICH,
      ),
      snippetListingCount: candidates.length,
      enrichedListingCount: enrichedAll.length,
      recoveryPoolCount: enrichedAll.length,
      verifiedListingCount: verified.length,
      targetListingCount: TARGET,
      confirmedPriceCount: listings.filter(
        (listing) => listing.priceConfidence === "confirmed",
      ).length,
      photoCount: listings.filter((listing) => listing.images.length > 0)
        .length,
      creditsUsed: null,
      sources,
      listings,
      searchEngineVersion: "15.0-ten-clean-listings",
