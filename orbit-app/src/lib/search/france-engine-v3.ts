...
function parseSurface(text: string, preferred?: number) {
  ...
  for (const match of text.matchAll(/(\d{2,4}(?:[.,]\d+)?)\s*(?:m²|m2|sqm)\b/gi)) {
    ...
    const around = norm(text.slice(Math.max(0, index - 110), index + match[0].length + 110));
...
function titleTokens(title: string) {
  return new Set(
    norm(title)
      .split(/[^a-z0-9]+/)
      .filter((token) => token.length >= 4 && !["maison", "vente", "vendre", "immobilier", "annonce", "france"].includes(token))
      .slice(0, 16),
  );
}
...
function sameListing(result: SearchResult, listing: Listing) {
  if (!result.url) return false;
  if (host(result.url) !== host(listing.url)) return false;
  if (normalizeUrl(result.url) === normalizeUrl(listing.url)) return true;
  return titleSimilarity(result.title ?? "", listing.title) >= 0.55;
}

async function recoverFromIndex(listing: Listing, criteria: Criteria) {
  const query = `site:${listing.source} "${listing.title.slice(0, 110)}"`;
  const results = await searx(query, 1);
  const matches = results.filter((result) => sameListing(result, listing)).slice(0, 4);
  let price: number | undefined;
  let image: string | undefined;
  for (const result of matches) {
    const text = `${clean(result.title)} ${clean(result.content)}`;
    if (!price) price = strictSnippetPrice(text, listing.surface ?? criteria.minSurface);
    if (!image) image = validImageUrl(result.img_src ?? result.thumbnail, result.url);
...
async function enrich(listing: Listing, criteria: Criteria): Promise<Listing> {
  ...
  const details = html ? parsePage(html, listing.url, criteria) : undefined;
  const surface = details?.surface ?? listing.surface;

  let price = details?.price;
  let priceConfidence: PriceConfidence = price ? "confirmed" : "none";
  let indexedImage = listing.discoveryImage;
  let indexedPrice = listing.discoveryPrice;

  if (!price || !(details?.images.length)) {
    const recovered = await recoverFromIndex(listing, criteria);
    indexedPrice = indexedPrice ?? recovered.price;
    indexedImage = indexedImage ?? recovered.image;
  }

  if (!price && indexedPrice) {
    price = indexedPrice;
  ...
  for (const image of directImages.slice(0, 4)) {
    const safe = validImageUrl(image, listing.url);
    if (!safe) continue;
    if (await imageReachable(safe)) checked.push(safe);
    else checked.push(safe);
    if (checked.length >= 2) break;
  }

  let imageConfidence: ImageConfidence = checked.length ? "confirmed" : "none";
  if (!checked.length && indexedImage) {
    const safe = validImageUrl(indexedImage, listing.url);
    if (safe) {
      checked.push(safe);
  ...
  const detectedKind = kindOf(combined);
  const propertyKind = detectedKind === "unknown" ? listing.propertyKind : detectedKind;
...
export async function franceSearchProxyV3(request: NextRequest) {
  try {
    ...
    if (FOREIGN_MARKERS.test(query) && !/\bfrance\b/i.test(query)) {
      return NextResponse.json(
        { success: false, error: "ORBIT recherche désormais uniquement des biens situés en France." },
        { status: 400 },
      );
    }
    ...
    for (let page = 1; page <= 5; page += 1) {
      const active = page <= 2 ? queries : queries.slice(0, 20);
    ...
    for (let i = 0; i < Math.min(MAX_ENRICH, candidates.length); i += BATCH) {
      ...
      enriched.push(...batch);
      const withPrice = uniqueListings(enriched).filter((item) => hardAccept(item, criteria));
      if (withPrice.filter((item) => item.images.length > 0).length >= TARGET) break;
    }
    ...
    return NextResponse.json({
      ...
      sources,
      listings,
      searchEngineVersion: "FR-3.0",
      searchProvider: "SearXNG-France",
      scope: "France-only",
    });
  } catch (error) {
    console.error("ORBIT France v3 search error:", error);
    return NextResponse.json(
      {
        success: false,
